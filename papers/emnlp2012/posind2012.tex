...
accuracy.  Our best model based on Euclidean co-occurence embedding
combines paradigmatic features with morphological and orthographic
features and achieves close to 80\% many-to-one accuracy on a 45-tag
million-word test corpus.
\end{abstract}
...
and the magnitudes represent the probability of occurence in the given
...
To learn syntactic categories, paradigmatic representations of word
context are combined with features of co-occuring words within the
co-occurence data embedding (CODE) framework of
\cite{globerson2007euclidean,maron2010sphere}.  We obtain many-to-one
accuracies up to .7662 using only distributional information and .7974
using morphological and orthographic features of words improving the
state-of-the-art in unsupervised part-of-speech tagging performance.

Section~\ref{sec:related} gives a detailed review of related work.
Section~\ref{sec:lm} desribes the dataset and the construction of the
substitute vectors.  Section~\ref{sec:code} describes co-occurence
...
Algorithms under this category can be further categorized into three
subgroups based on the learning algorithm.  The first subgroup
represents each word type with its context vector and clusters these
vectors accordingly \cite{Schutze:1995:DPT:976973.976994}.  Work in
modeling child syntactic category acquisition has generally followed
...
can gain from 4\% (.62 to .66 with a 1M word corpus) in
cross-validated many-to-one accuracy.  However
\cite{Christodoulopoulos:2010:TDU:1870658.1870714} shows that the
older one-tag-per-word models such as
...
the baseline Brown model.
\cite{christodoulopoulos-goldwater-steedman:2011:EMNLP} defines a
type-based Bayesian multinomial mixture model in which each word
instance is generated from the corresponding word type mixture
...
\subsection{Paradigmatic representations of context}

The paradigmatic context representation in our work is closely related
to the first and second order co-occurrence methods in
\cite{Schutze:1995:DPT:976973.976994}.  \cite{sahlgren2006word} gives
a detailed analysis of paradigmatic and syntagmatic relations in the
context of word-space models used to represent word meaning.  He finds
that in word-spaces built by paradigmatic uses of context, more
nearest neighbors share the same part-of-speech.  Our work is
different in that we represent word contexts, not word types using
paradigmatic features and we use a statistical language model to
compute accurate substitute probabilities rather than relying on an
ad-hoc selection of neighbors.
...
features of the context as seen in the examples given in the example
below.  Top substitutes for the word ``the'' consist of words that can
act as determiners.  Top substitutes for ``board'' are not only nouns,
but specifically nouns compatible with the semantic context.

\begin{quotation}
\noindent {\em ``Pierre Vinken, 61 years old, will join the  board as a
  nonexecutive director Nov.~29.''}\\
{\bf the:} its (.9011), the (.0981), a (.0006), $\ldots$\\
{\bf board:} board (.4288), company (.2584), firm (.2024), bank
(.0731), $\ldots$
\end{quotation}
...
Bigrams (Sec.~\ref{sec:bigram}) & .7344 (.0091) & .6566 (.0049) \\
Partitions (Sec.~\ref{sec:rpart}) & .7594 (.0085) & .6716 (.0052) \\
Substitutes (Sec.~\ref{sec:wordsub}) & .7662 (.0066) & .6811 (.0044) \\
\hline
\end{tabular}
\begin{tabular}{|l|l|l|}
\hline
...
\cite{bergkirkpatrick-klein:2010:ACL} & .755 & -\\ % Intresting in  christo paper:73.9/67.7
\cite{Christodoulopoulos:2010:TDU:1870658.1870714} & .761 & .688\\
\cite{blunsom-cohn:2011:ACL-HLT2011} & .775 & .697\\
Substitutes and Features (Sec.~\ref{sec:feat}) & .7974 (.0073) & .7212 (.0052) \\
& & \\
& & \\
\hline
\end{tabular}
...
and the V-measure is .6566 (.0049)  for 45 clusters.  The following
...
dimensions, $Z=0.166$) the many-to-one accuracy is .7594 (.0085) and
the V-measure is .6716 (.0052).
...
(.0066) and a V-measure of .6811 (.0044).
...
result is fairly robust as long as the training algorithm can observe
...
\cite{Clark:2003:CDM:1067807.1067817} demonstrates that using
morphological and orthographic features significantly improves
...
In order to accomodate multiple feature types the CODE model needs to
...
of the Penn Treebank using default settings, and the suggested $b$
value of 200.  The program induced 7 suffix types that are present in
a total of 11,917 word types.  These suffixes were input to S-CODE as
morphological features whenever the associated word types were
sampled.
...
score of the random-substitute model to .7974 (.0073) and V-measure to
.7212 (.0052).  Both these results improve the state-of-the-art in
...
than homogeneity (multiple large entries in a column).  The algorithm
tends to split large word classes into several clusters.  Some
examples are:
\begin{itemize}
...
the Penn Treebank category they belong to.  Thus it can be argued that
metrics that emphasize homogeneity such as \mto are more appropriate
in this context than metrics that average homogeneity and completeness
such as \vm as long as the number of clusters is controlled.
...
\item We introduced substitute vectors as paradigmatic representations
  ...
  acquisition.
\item We demonstrated that paradigmatic representations give superior
  results when compared to bigram based representations using the
  S-CODE learning framework.
...
\item All our code and data, including the substitute vectors for the
  one million word Penn Treebank Wall Street Journal dataset, are
  available at the authors' website at \url{http://xxx.xxx.xxx}.
\end{itemize}
