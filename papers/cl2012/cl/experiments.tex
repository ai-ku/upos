...
In this section we run our model on three different representation
that are described in Section~\ref{sec:cooc} to determine their
performance on the English POS induction problem.  We extend the best
performing model with extra orthographic and morphological features to
improve its performance further.  Finally, we apply our model to
languages other than English.

Section~\ref{sec:expset} details the test corpus and experiment
parameters used in the rest of this work.
Section~\ref{sec:clustering-w} is presents the sensitivity analysis of
model parameters.  Section~\ref{sec:clustering-w},
\ref{sec:clustering-w} and \ref{sec:clustering-concatenation} aim to
compare performance of models on ${\bf W}$, ${\bf C}$ and ${\bf
  W}\oplus{\bf C}$, respectively.  Section~\ref{sec:feat} explores
morphological and orthographic features as additional sources of
information for POS induction of word types and its results improve
the state-of-the-art in the field of POS induction.
Section~\ref{sec:pvss} compares our paradigmatic representation of the
word context to previously described syntagmatic representations of
the word context for word types.  Finally, Section~\ref{sec:multilang}
extends the language and corpus coverage by applying the best
performing model to 19 corpora in 15 languages.
