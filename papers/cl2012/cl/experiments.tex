...
The S-CODE uses stochastic gradient ascent to find the $\phi_x,
\psi_y$ embeddings for word and random-substitute in these pairs on a
single 25-dimensional sphere.  The algorithm cycles through the data
until we get approximately 50 million updates.  The resulting $\phi_x$
vectors are clustered using an instance weighted k-means algorithm.
Cluster-id for each $\phi_x$ is assigned as the predicted cluster-id
for word type $X$.  Using the default settings and sampling 64
