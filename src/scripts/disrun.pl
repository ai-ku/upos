...
my $scode = "scode -r $sc_restart -i $sc_niter -d $ndim -z $Z -s $seed -p $phi -u $nu";
...
my $cmd = "$wordsub | $scode 2> $sc_err | $scode2kmeans | $kmeans 2> $km_err | $kmeans2eval | $score 2> $ev_err";
my $tm = time;
system($cmd);
$tm = time - $tm;
